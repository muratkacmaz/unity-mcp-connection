            ...
            while (isRunning && client.Connected)
            {
                try
                {
                    ...
                    if (!handshakeComplete)
                    {
                        ...
                        {
                            string key = Regex.Match(handshakeRequest, "Sec-WebSocket-Key: (.*)").Groups[1].Value.Trim();
                        ...
                        {
                            Debug.LogError("Invalid WebSocket handshake request");
                            break;
                        }
                    }
                    else
                ...
                {
                    Debug.LogError($"Error handling client data: {e.Message}");
                    break;
                }
            }
        }
